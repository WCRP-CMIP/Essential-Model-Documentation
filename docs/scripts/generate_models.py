...
def safe_get_label(obj, default=""):
    """Safely extract the best label from any CV object or string.
    Priority: ui_label -> validation_key -> @id (last segment)."""
    if obj is None:
        return default
    if isinstance(obj, str):
        return obj.split("/")[-1].split(":")[-1].replace("-", " ").replace("_", " ").title() or default
    if isinstance(obj, dict):
        return (
            obj.get("ui_label") or
            obj.get("validation_key") or
            obj.get("@id", "").split("/")[-1].split(":")[-1].replace("-", " ").replace("_", " ").title()
        ) or default
    return default
...
def parse_family(f):
    ...
    if isinstance(f, str):
        return {
            ...
            "description": "",
            "page": f"../04_Earth_System_Model_Families/{f}.html",
        }
    if isinstance(f, dict):
        ...
        return {
            ...
            "page": f"../04_Earth_System_Model_Families/{fid}.html",
        }
    return None


def extract_realm_from_config_id(config_id):
    """Extract the scientific domain/realm from a component_config ID string.
    
    Config IDs follow the pattern: realm-name-version-hNNN-vNNN
    e.g. 'atmosphere-arpege-climat-version-6-3-h100-v100'
    """
    if not config_id:
        return ""
    # Known realm prefixes (longest first to avoid partial matches)
    realm_prefixes = [
        "ocean-biogeochemistry", "atmospheric-chemistry", "land-surface",
        "sea-ice", "land-ice", "atmosphere", "ocean", "aerosol",
    ]
    for prefix in realm_prefixes:
        if config_id.startswith(prefix + "-"):
            return prefix
    # Fallback: first segment
    return config_id.split("-")[0]
...
def parse_component_config(cc):
    ...
    if isinstance(cc, dict):
        ...
        cc_id = cc.get("@id", "")
        realm = extract_realm_from_config_id(cc_id)
        return {
            "id": cc_id,
            "name": cc.get("ui_label") or cc_id.replace("-", " ").replace("_", " ").title(),
            "description": cc.get("description", ""),
            "component": safe_get_label(mc),
            "h_grid": safe_get_label(hcg),
            "h_grid_id": h_id,
            "h_grid_page": f"../05_Horizontal_Computational_Grids/{h_id}.html" if h_id else "",
            "v_grid": safe_get_label(vcg),
            "v_grid_id": v_id,
            "v_grid_page": f"../06_Vertical_Computational_Grids/{v_id}.html" if v_id else "",
            "realm": realm,
        }
    return None
...
def parse_nested_list(items):
    ...
    Handles both flat lists of strings/dicts and nested sublists."""
    if is_none_value(items):
        return []
    if isinstance(items, list):
        result = []
        for item in items:
            if isinstance(item, list):
                ...
                label = safe_get_label(item) if isinstance(item, dict) else str(item)
                result.append(label)
        return result
    return []


def parse_dynamic_component(comp):
    """Parse a dynamic/prescribed/omitted component entry."""
    if is_none_value(comp):
        return None
    if isinstance(comp, str):
        return {
            "id": comp,
            "name": comp.replace("-", " ").replace("_", " ").title(),
            "description": "",
            "aliases": [],
        }
    if isinstance(comp, dict):
        ...
        return {
            "id": cid,
            "name": comp.get("ui_label") or comp.get("description") or cid.replace("-", " ").replace("_", " ").title(),
            "description": comp.get("description", ""),
            "aliases": aliases,
            "validation_key": comp.get("validation_key", ""),
        }
    return None


def parse_calendar(cal):
    """Parse calendar data (can be a string, dict, or list)."""
    if is_none_value(cal):
        return None
    if isinstance(cal, str):
        return {"id": cal, "name": cal.replace("-", " ").title(), "description": ""}
    if isinstance(cal, dict):
        return {
            "id": cal.get("@id", ""),
            "name": cal.get("ui_label") or cal.get("@id", "").replace("-", " ").title(),
            "description": cal.get("description", ""),
            "alias": cal.get("alias", ""),
        }
    if isinstance(cal, list):
        return [parse_calendar(c) for c in cal if not is_none_value(c)]
    return None
...
def prepare_template_context(data):
    ...
    dynamic_components = parse_list_items(data.get("dynamic_components"), parse_dynamic_component)
    prescribed_components = parse_list_items(data.get("prescribed_components"), parse_dynamic_component)
    omitted_components = parse_list_items(data.get("omitted_components"), parse_dynamic_component)
