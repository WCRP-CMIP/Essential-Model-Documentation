...
REALM_COLORS = {
    ...
    "ocean": "#0ea5e9",                # Cyan
    "land-surface": "#22c55e",         # Green
    "sea-ice": "#67e8f9",              # Light cyan
    "land-ice": "#bfdbfe",             # Very light blue
    "aerosol": "#a78bfa",              # Purple
    "atmospheric-chemistry": "#c084fc", # Light purple
    "ocean-biogeochemistry": "#06b6d4", # Teal
    "default": "#94a3b8"               # Gray
}
...
def get_realm_color(realm_id):
    """Get color for a realm."""
    if not realm_id:
        return REALM_COLORS["default"]
    realm_lower = realm_id.lower()
    for key, color in REALM_COLORS.items():
        if key == realm_lower or key.replace("-", "") == realm_lower.replace("-", ""):
            return color
    return REALM_COLORS["default"]
...
def build_chord_data():
    ...
    model_names = []
    realm_names = list(REALM_COLORS.keys())[:-1]  # Exclude 'default'
