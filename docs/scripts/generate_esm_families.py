#!/usr/bin/env python3
"""Generate Earth System Model family HTML pages from JSON-LD metadata."""
...
OUTPUT_DIR = SCRIPT_DIR.parent / "1_Explore_The_EMD" / "ESM_Families"
...
def prepare_template_context(data):
    ...
    return {
        ...
        "types": types,
        "family_type": "Earth System Model Family",
...
def main():
    print("Earth System Model Family Page Generator")
    print("=" * 40)
    ...
    # Get all model_family entries and filter for model/source families
    all_families = fetch_data("model_family", depth=2)
    families = [f for f in all_families if isinstance(f, dict) and f.get("family_type") == "model"]
    
    print(f"Found {len(families)} ESM families")
    
    if not families:
        print("No ESM families found - check data source")
        return 0
    
    success = 0
    with tqdm(families, desc="Generating ESM families", unit="file") as pbar:
        for data in pbar:
            if process_family(env, template, data, pbar):
                success += 1
    
    print(f"Done: {success}/{len(families)} ESM families generated")
    return 0
