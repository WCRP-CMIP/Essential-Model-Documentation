...
function gridCard(kind, id, doc, rows, usedBy) {
  ...
  return el("div", { class: `grid-card grid-${kind}` }, [
    el("div", { class: "grid-card-head" }, [
      ...
      usedBy.size && el("span", { class: "grid-usedby" }, [...usedBy].map(realmLabel).sort().join(", ")),
    ]),
    !isNone(doc.description) && el("p", { class: "grid-desc" }, doc.description),
...
export async function mountGrids(root, model, { base }) {
  ...
  root.appendChild(el("section", { class: "card grids" }, [
    el("h2", { class: "card-title" }, "Grids"),
    el("p", { class: "card-sub" }, "Horizontal (H) and vertical (V) computational grids used by this model's components."),
    grid,
  ]));
}
