...
//
// Produces two cards:
//   1. "Model family" — institution, domains, website, refs for model.family
//   2. "Components in detail" — one row per realm: the model_component's
//      description, code base, references, and its component family.
...
function familyCard(fam) {
  ...
  return el("section", { class: "card family-card" }, [
    el("h2", { class: "card-title" }, "Model family"),
    el("div", { class: "family-head" }, [
      el("span", { class: "family-name" }, fam.ui_label || fam.validation_key || short(fam["@id"])),
      links.length && el("span", { class: "family-links" }, links),
    ]),
    !isNone(fam.description) && el("p", { class: "family-desc" }, fam.description),
...
function componentRow(realm, comp, compFamily, base, ccId) {
  ...
  return el("div", { class: "comp-row", style: `--realm:${color}` }, [
    el("div", { class: "comp-row-head" }, [
      ...
      comp && el("span", { class: "comp-name" }, comp.name || comp.ui_label || short(comp["@id"])),
    ]),
    comp && !isNone(comp.description) && el("p", { class: "comp-desc" }, comp.description),
    el("div", { class: "comp-meta" }, [
      compFamily && el("span", { class: "comp-tag" }, [
        ...
        !isNone(compFamily.website)
          ...
          : (compFamily.ui_label || compFamily.validation_key || short(compFamily["@id"])),
        !isNone(compFamily.primary_institution) && el("span", { class: "muted" }, ` · ${orgLabel(compFamily.primary_institution)}`),
      ]),
      codeBaseNode && el("span", { class: "comp-tag" }, codeBaseNode),
...
export async function mountLinkedInfo(root, model, { base }) {
  const resolver = new Resolver(base);

  // ---- model family
  if (model.family && !isNone(model.family)) {
    try {
      const fam = await resolver.fetchDoc("model_family", short(model.family));
      const card = familyCard(fam);
  ...
  const section = el("section", { class: "card components-detail" }, [
    el("h2", { class: "card-title" }, "Components in detail"),
    el("p", { class: "card-sub" }, "The code implementing each realm, its provenance, and the component family it belongs to."),
  ]);
