...
export function mountCrsDiagram(root, model) {
  ...
  // arrowhead markers for couplings (double-headed)
  const defs = svg("defs");
  for (const [id, pts, refX] of [["crs-ah-a", "0,0 6,3 0,6", 5.5], ["crs-ah-b", "6,0 0,3 6,6", 0.5]]) {
    const m = svg("marker", { id, markerWidth: 6, markerHeight: 6, refX, refY: 3, orient: "auto" });
    m.appendChild(svg("polygon", { points: pts, class: "crs-couple-head" }));
    defs.appendChild(m);
  }
  s.appendChild(defs);
  ...
  for (const [a, b] of couplingPairs) {
    ...
    // two short half-arrows meeting at the curve's apex, tangent to it (bidirectional)
    // tangent at the quadratic midpoint (t=0.5) points along (pb - pa)
    const HALF = 14;
    arcs.appendChild(svg("line", { x1: mx, y1: my, x2: mx + ux * HALF, y2: my + uy * HALF, class: "crs-couple-arm", "marker-end": "url(#crs-ah-a)" }));
    arcs.appendChild(svg("line", { x1: mx, y1: my, x2: mx - ux * HALF, y2: my - uy * HALF, class: "crs-couple-arm", "marker-end": "url(#crs-ah-b)" }));
  }
  s.appendChild(arcs);
